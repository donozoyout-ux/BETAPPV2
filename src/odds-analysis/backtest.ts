...
export function runOddsBacktest(matches: HistoricalOddsMatch[], config: AnalysisConfig) {
  ...
  return {
    ...
    eligibleAnalyses: eligible.length,
    movementClassCounts: countByClass,
    hitRatesByScoreBucket: bucketize(eligible, (record) => scoreBucket(record.item.score), (record) => record.hit),
    hitRatesByConfidenceBucket: bucketize(eligible, (record) => scoreBucket(record.item.modelConfidence.score), (record) => record.hit),
    bookmakerCountBuckets: bucketize(eligible, (record) => String(record.item.bookmakerCount), (record) => record.hit),
    dataQualityBuckets: bucketize(records, (record) => record.item.dataQuality.grade, (record) => record.hit),
