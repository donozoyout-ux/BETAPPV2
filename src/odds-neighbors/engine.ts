import { historicalNeighbors, scoreNeighbor } from './reliability.js';
...
export function findPastTwins(target: { competitionId: string; country?: string | null; route: OddsRoute }, candidates: HistoricalNeighborInput[], options: SearchOptions,
  config: OddsNeighborConfig = oddsNeighborConfig): HistoricalTwin[] {
  const filtered = [...new Map(candidates.map(c => [c.matchId,c])).values()].filter((candidate) => candidate.route.marketType === target.route.marketType && candidate.route.marketName === target.route.marketName
  ...
  const scored = filtered.map((candidate) => ({ candidate, score: routeDistance(target.route, candidate.route, target.competitionId, candidate.competitionId, config) }))
    ...
    .sort((a, b) => (scoreNeighbor(target,b.candidate)?.similarityScore ?? 0)-(scoreNeighbor(target,a.candidate)?.similarityScore ?? 0) || (options.mode === 'CLOSEST_NEIGHBORS' ? a.score.distance - b.score.distance || a.candidate.kickoffAt.getTime() - b.candidate.kickoffAt.getTime() || a.candidate.matchId.localeCompare(b.candidate.matchId)
      : a.candidate.kickoffAt.getTime() - b.candidate.kickoffAt.getTime() || a.candidate.matchId.localeCompare(b.candidate.matchId)))
    .slice(0, options.limit);
  return scored.map(({ candidate, score }, index) => ({ ...candidate, ...scoreNeighbor(target,candidate)!, exampleId: candidate.matchId, distance: score.distance, similarity: score.similarity,
...
export function calculateEvidenceStrength(twins: HistoricalTwin[], gaps: Array<{ wilson: { lower: number | null; upper: number | null }; sampleSize: number }>, baselineSample: number): EvidenceStrength {
  ...
  const width = Math.max(...gaps.map((item) => item.wilson.lower == null || item.wilson.upper == null ? 1 : item.wilson.upper - item.wilson.lower), 0);
...
export function buildOddsIntelligence(target: MatchOutcomeData & { route: OddsRoute }, candidates: HistoricalNeighborInput[], options: SearchOptions,
  ...
  const twins = findPastTwins({ competitionId: target.competitionId, country: target.country ?? null, route: target.route }, eligible, options, config);
  const maps = evidenceGaps(twins, eligible, target.competitionId, config.minimumBaselineSample);
  const legacyStrength = calculateEvidenceStrength(twins, maps, Math.max(0, ...maps.map((row) => row.baselineSampleSize)));
  const reliability = historicalNeighbors(target, twins);
  const cap: EvidenceStrength = reliability.status === 'STRONG_EVIDENCE' ? 'HIGH' : reliability.status === 'MODERATE_EVIDENCE' ? 'MEDIUM' : reliability.status === 'WEAK_EVIDENCE' ? 'LOW' : 'VERY_LOW';
  const levels: EvidenceStrength[] = ['VERY_LOW','LOW','MEDIUM','HIGH'];
  const evidenceStrength = levels[Math.min(levels.indexOf(legacyStrength),levels.indexOf(cap))]!;
  const conflictCheck = conflicts(target.route,maps).map(c => c.source === 'ODDS_TWINS' && reliability.supportingEvidence === 'NONE'
    ? {...c,state:'UNAVAILABLE' as const,reason:'Örnek sayısı veya bağlam yeterli değil; yalnız bilgi amaçlı.'} : c);
  return { historicalNeighbors: reliability, match: { id: target.matchId, kickoffAt: target.kickoffAt, league: target.league, homeTeam: target.homeTeam, awayTeam: target.awayTeam },
    ...
    searchMode: options.mode, pastTwins: twins, resultMap: maps, evidenceGap: maps, evidenceStrength, conflictCheck,
