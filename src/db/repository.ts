...
export class FootballRepository {
  ...
  async liveOddsMatchDetail(matchId: string) {
    ...
    if(!table)return {...match.rows[0],observations:[],liveOddsStatus:'NOT_CONFIGURED',observationCount:0};
    ...
    return {...match.rows[0],observations:observations.rows,liveOddsStatus:tracking?.last_status??(observations.rowCount?'AVAILABLE':'NO_DATA'),
      observationCount:observations.rowCount??0,lastCaptureAt:tracking?.last_capture_at??observations.rows[0]?.captured_at??null};
  }
}
