...
export function renderDashboard(data: DashboardData): string {
  ...
  const oddsIntelligenceCards = oddsIntelligence.length ? oddsIntelligence.map((entry) => {
    ...
    const resultRows = map.filter((item) => finiteNumber(item.sampleSize) > 0).slice(0, 4).map((item) =>
      `<li>${escapeHtml(translateMarket(item.market))} ${item.line == null ? '' : escapeHtml(item.line)} · ${escapeHtml(translateSelection(item.selection))}: <strong>${escapeHtml(item.positiveCount)}/${escapeHtml(item.sampleSize)}</strong> (${historicalRateText(item.positiveCount,item.sampleSize,item.positiveRate == null ? null : Number((Number(item.positiveRate)*100).toFixed(1)))})</li>`).join('');
...
export function renderLiveOddsDetail(data:Record<string,unknown>,sheetUrl?:string):string{
  const observations=Array.isArray(data.observations)?data.observations as Array<Record<string,unknown>>:[];
  const score=(item:Record<string,unknown>)=>item.score_home==null||item.score_away==null?'—':String(item.score_home)+':'+String(item.score_away);
  const num=(value:unknown)=>value==null?'—':String(value);
  const rows=observations.map((item)=>'<tr><td>'+escapeHtml(item.match_minute??item.minute_label??'—')+'</td><td>'+escapeHtml(score(item))+'</td><td>'+escapeHtml(item.period)+'</td><td>'+escapeHtml(item.bookmaker??'Bookmaker unknown')+'</td><td>'+num(item.ah_live_home)+' / '+num(item.ah_live_line)+' / '+num(item.ah_live_away)+'</td><td>'+num(item.one_x_two_live_home)+' / '+num(item.one_x_two_live_draw)+' / '+num(item.one_x_two_live_away)+'</td><td>'+num(item.ou_live_over)+' / '+num(item.ou_live_line)+' / '+num(item.ou_live_under)+'</td></tr>').join('');
  const status=String(data.liveOddsStatus??'NO_DATA');
  const sheetUrlHtml=sheetUrl?'<a class="button" href="'+escapeHtml(sheetUrl)+'" target="_blank" rel="noreferrer">Open Google Sheet</a>':'';
  return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Live Odds · '+escapeHtml(data.home_team)+' vs '+escapeHtml(data.away_team)+'</title><style>body{max-width:1100px;margin:36px auto;padding:0 18px;background:#09110d;color:#edf7f0;font:15px system-ui}a{color:#5ee59a}.card{background:#122019;border:1px solid #294033;border-radius:14px;padding:18px;margin:18px 0}.button{display:inline-block;padding:10px 14px;border-radius:8px;background:#173e27}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:10px;border-bottom:1px solid #294033}th{font-size:12px;color:#a8b9ae}.scroll{overflow:auto}</style></head><body><a href="/">← Dashboard</a><h1>'+escapeHtml(data.home_team)+' – '+escapeHtml(data.away_team)+'</h1><article class="card"><strong>LIVE ODDS · Nowgoal</strong><p>Source: Nowgoal · PostgreSQL: '+escapeHtml(data.postgresStatus??status)+' · Google Sheets: '+escapeHtml(data.sheetSyncStatus??'NOT_CONFIGURED')+'</p><p>Capture status: '+escapeHtml(status)+' · Observations: '+escapeHtml(data.observationCount??0)+' · Bookmakers: '+escapeHtml(data.bookmakerCount??0)+' · FT: '+escapeHtml(data.ftCount??0)+' · HT: '+escapeHtml(data.htCount??0)+' · Last capture: '+escapeHtml(data.lastCaptureAt??'No data')+'</p>'+sheetUrlHtml+'</article><section class="card"><h2>History observations</h2><div class="scroll"><table><thead><tr><th>Minute</th><th>Score</th><th>Period</th><th>Bookmaker</th><th>AH live</th><th>1X2 live</th><th>O/U live</th></tr></thead><tbody>'+(rows||'<tr><td colspan="7">No live odds observations yet.</td></tr>')+'</tbody></table></div></section></body></html>';
}
