...
2. Her timestamp'te son bilinen seçim değerleriyle piyasa state'i yeniden kurulur. İlk ve son tamamlanmış state açılış ve güncel state'tir. Bookmaker bazında `completeStateCount`, `openingCompleteAt`, `currentCompleteAt` ve state'lerin ayrık olup olmadığı izlenir. Her seçimin yeterli zaman kapsamı yoksa sonuç diagnostic kalır. Eksik 1X2 veya eksik iki yönlü piyasa normalize edilmez.
...
- Probability calibration bütün 1X2 seçimlerini binary event olarak kullanır; market movement hit-rate raporu yalnız qualified SUPPORT/STRONG_SUPPORT sinyallerini kullanır. Leakage audit sayaçları her historical girdiden hesaplanır ve `unsafeSnapshotsUsed` sıfır olmak zorundadır.
