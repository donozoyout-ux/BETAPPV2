...
export function analyzeOdds(matchId: string, kickoffAt: Date, snapshots: OddsSnapshot[],
  ...
  const items = [...groups.values()].map((votes): AnalysisItem => {
    ...
    return {
      ...
      completeStateBookmakerCount: consensus.completeStateBookmakerCount,
      minimumCompleteStateCount: consensus.minimumCompleteStateCount,
  ...
  return {
    ...
    metadata: { safeSnapshotCount: valid.length, excludedAfterKickoff, unsafeSnapshotsUsed:
      valid.filter((snapshot) => snapshot.capturedAt >= kickoffAt).length,
