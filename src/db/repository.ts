...
export class FootballRepository {
  ...
  async databaseHealth() {
    ...
    requiredIndexes.push('odds_analysis_runs_match_created_idx','odds_analysis_items_run_idx','odds_analysis_backtests_created_idx');
  ...
  async dashboardData(timeZone: string) {
    const [matches, providers, qualification, cornerAnalyses, oddsAnalyses, datasetAudit, validation, backfill] = await Promise.all([
      this.pool.query(
    ...
    return { matches: matches.rows, providers: providers.rows, qualification: qualification.rows, cornerAnalyses: cornerAnalyses.rows,
      oddsAnalyses: oddsAnalyses.rows,
