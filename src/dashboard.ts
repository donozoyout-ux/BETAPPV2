...
export function renderDashboard(data: DashboardData): string {
  ...
  const rows = data.matches.length
    ? data.matches.map((match) => {
        ...
        return `<tr><td>${escapeHtml(new Date(String(match.kickoff_at)).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Istanbul' }))}</td>
          ...
          <td>${escapeHtml(match.status)}</td><td>${labels || 'Henüz yok'} · <a href="/matches/${escapeHtml(match.id)}/live-odds">Live Odds</a></td></tr>`;
      }).join('')
  ...
  return `<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    ...
    section{margin-top:34px}table{width:100%;border-collapse:collapse;background:var(--panel);border-radius:14px;overflow:hidden}th,td{text-align:left;padding:14px;border-bottom:1px solid var(--line);vertical-align:top}th{color:var(--muted);font-size:12px;text-transform:uppercase}@media(max-width:700px){th:nth-child(2),td:nth-child(2){display:none}}
...
export function renderLiveOddsDetail(data:Record<string,unknown>,sheetUrl?:string):string{
  ...
  const rows=observations.map((item)=>'<tr><td>'+escapeHtml(item.match_minute??item.minute_label??'—')+'</td><td>'+escapeHtml(score(item))+'</td>'+
    '<td>'+escapeHtml(item.period)+'</td><td>'+escapeHtml(item.bookmaker??'Bookmaker unknown')+'</td>'+
    '<td>'+num(item.ah_live_home)+' / '+num(item.ah_live_line)+' / '+num(item.ah_live_away)+'</td>'+
    '<td>'+num(item.one_x_two_live_home)+' / '+num(item.one_x_two_live_draw)+' / '+num(item.one_x_two_live_away)+'</td>'+
    '<td>'+num(item.ou_live_over)+' / '+num(item.ou_live_line)+' / '+num(item.ou_live_under)+'</td></tr>').join('');
  const status=String(data.liveOddsStatus??'NO_DATA');
  const sheetUrlHtml=sheetUrl?'<a class="button" href="'+escapeHtml(sheetUrl)+'" target="_blank" rel="noreferrer">Sheet verisini gör</a>':'';
  return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'+
    '<title>Live Odds · '+escapeHtml(data.home_team)+' vs '+escapeHtml(data.away_team)+'</title><style>'+
    'body{max-width:1100px;margin:36px auto;padding:0 18px;background:#09110d;color:#edf7f0;font:15px system-ui}a{color:#5ee59a}.card{background:#122019;border:1px solid #294033;border-radius:14px;padding:18px;margin:18px 0}.button{display:inline-block;padding:10px 14px;border-radius:8px;background:#173e27}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:10px;border-bottom:1px solid #294033}th{font-size:12px;color:#a8b9ae}.scroll{overflow:auto}</style></head><body>'+
    '<a href="/">← Dashboard</a><h1>'+escapeHtml(data.home_team)+' – '+escapeHtml(data.away_team)+'</h1>'+
    '<article class="card"><strong>LIVE ODDS · Nowgoal</strong><p>Status: '+escapeHtml(status)+' · Last minute: '+escapeHtml(observations[0]?.match_minute??observations[0]?.minute_label??'No data')+
    ' · Captured at: '+escapeHtml(data.lastCaptureAt??'No data')+
    ' · New observations: '+Number(data.observationCount??0)+' · Bookmaker: '+escapeHtml(observations[0]?.bookmaker??'Unknown')+'</p>'+sheetUrlHtml+'</article>'+
    '<section class="card"><h2>History observations</h2><div class="scroll"><table><thead><tr><th>Minute</th><th>Score</th><th>Period</th><th>Bookmaker</th><th>AH live</th><th>1X2 live</th><th>O/U live</th></tr></thead>'+
    '<tbody>'+(rows||'<tr><td colspan="7">No live odds observations yet.</td></tr>')+'</tbody></table></div></section></body></html>';
}
